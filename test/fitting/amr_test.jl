...
@testset "AMR Fitting and Sampling" begin
    ...
    @testset "fit_sfh" begin
        ...
        fαresult = SFH.fit_sfh(SFH.LinearAMR(MHmodel.α, MHmodel.β, MHmodel.T_max, (false, true)),
    ...
    @testset "fit_sfh_newton" begin
        x0 = SFH.construct_x0_mdf(logAge, convert(T, T_max); normalize_value=sum(x))
        # Run fit on perfect, noise-free data
        result = SFH.fit_sfh_newton(SFH.update_params(MHmodel, (MHmodel.α + 0.5, MHmodel.β + 1.0)),
                                    SFH.update_params(disp, (disp.σ + 0.1,)),
                                    smodels, sdata2, logAge, MH,
                                    x0=x0)
        @test result.mle.μ ≈ true_vals # With no error, we should converge exactly
        ...
        # Run fit on noisy data
        rresult = SFH.fit_sfh_newton(SFH.update_params(MHmodel, (MHmodel.α + 0.5, MHmodel.β + 1.0)),
                                     SFH.update_params(disp, (disp.σ + 0.1,)),
                                     smodels, sdata, logAge, MH, x0=x0)
        # Test that MLE and MAP results are within 3σ of the true answer for all parameters
        @test all(isapprox(rresult.mle.μ[i], true_vals[i];
        ...
        fαresult = SFH.fit_sfh_newton(SFH.LinearAMR(MHmodel.α, MHmodel.β, MHmodel.T_max, (false, true)),
                                      SFH.GaussianDispersion(disp.σ, (true,)),
                                      smodels, sdata, logAge, MH, x0=x0)
        @test fαresult.map.μ[end-2] ≈ MHmodel.α
        @test fαresult.mle.μ[end-2] ≈ MHmodel.α
        ...
        # Run with fixed parameters on noisy data, verify that best-fit values are unchanged
        fresult = SFH.fit_sfh_newton(SFH.LinearAMR(MHmodel.α, MHmodel.β, MHmodel.T_max, (false, false)),
                                     SFH.GaussianDispersion(disp.σ, (false,)),
                                     smodels, sdata, logAge, MH, x0=x0)
        @test fresult.map.μ[end-2:end] ≈ [MHmodel.α, MHmodel.β, disp.σ]
        @test fresult.mle.μ[end-2:end] ≈ [MHmodel.α, MHmodel.β, disp.σ]
    ...
    @testset "fit_sfh_fisher" begin
        x0 = SFH.construct_x0_mdf(logAge, convert(T, T_max); normalize_value=sum(x))
        # Run fit on perfect, noise-free data
        result = SFH.fit_sfh_fisher(SFH.update_params(MHmodel, (MHmodel.α + 0.5, MHmodel.β + 1.0)),
                                    SFH.update_params(disp, (disp.σ + 0.1,)),
                                    smodels, sdata2, logAge, MH,
                                    x0=x0)
        @test result.mle.μ ≈ true_vals # With no error, we should converge exactly
        ...
        # Run fit on noisy data
        rresult = SFH.fit_sfh_fisher(SFH.update_params(MHmodel, (MHmodel.α + 0.5, MHmodel.β + 1.0)),
                                     SFH.update_params(disp, (disp.σ + 0.1,)),
                                     smodels, sdata, logAge, MH, x0=x0)
        # Test that MLE and MAP results are within 3σ of the true answer for all parameters
        @test all(isapprox(rresult.mle.μ[i], true_vals[i];
        ...
        # Run with fixed α, to check that free parameter indexing is working properly
        fαresult = SFH.fit_sfh_fisher(SFH.LinearAMR(MHmodel.α, MHmodel.β, MHmodel.T_max, (false, true)),
                                      SFH.GaussianDispersion(disp.σ, (true,)),
                                      smodels, sdata, logAge, MH, x0=x0)
        @test fαresult.map.μ[end-2] ≈ MHmodel.α
        @test fαresult.mle.μ[end-2] ≈ MHmodel.α
        ...
        # Run with fixed parameters on noisy data, verify that best-fit values are unchanged
        fresult = SFH.fit_sfh_fisher(SFH.LinearAMR(MHmodel.α, MHmodel.β, MHmodel.T_max, (false, false)),
                                     SFH.GaussianDispersion(disp.σ, (false,)),
                                     smodels, sdata, logAge, MH, x0=x0)
        @test fresult.map.μ[end-2:end] ≈ [MHmodel.α, MHmodel.β, disp.σ]
        @test fresult.mle.μ[end-2:end] ≈ [MHmodel.α, MHmodel.β, disp.σ]
    ...
    @testset "LogarithmicAMR" begin
        ...
        @testset "fit_sfh_newton" begin
            x0 = SFH.construct_x0_mdf(logAge, convert(T,T_max); normalize_value=sum(logx))
            # Run fit on perfect, noise-free data
            result = SFH.fit_sfh_newton(SFH.update_params(logMHmodel, (logMHmodel.α + 1e-4, logMHmodel.β + 1e-4)),
                                        SFH.update_params(disp, (disp.σ + 0.1,)),
                                        smodels, logsdata2, logAge, MH,
                                        x0=x0)
            @test result.mle.μ ≈ log_true_vals # With no error, we should converge exactly
            ...
            # Run fit on noisy data
            rresult = SFH.fit_sfh_newton(SFH.update_params(logMHmodel, (logMHmodel.α + 1e-4, logMHmodel.β + 1e-4)),
                                         SFH.update_params(disp, (disp.σ - 0.1,)),
                                         smodels, logsdata, logAge, MH, x0=x0)
            # Test that MLE and MAP results are within 3σ of the true answer for all parameters
            @test all(isapprox(rresult.mle.μ[i], log_true_vals[i];
            ...
            # Run with fixed parameters on noisy data, verify that best-fit values are unchanged
            fresult = SFH.fit_sfh_newton(SFH.LogarithmicAMR(logMHmodel.α, logMHmodel.β, logMHmodel.T_max,
                                                             logMHmodel.MH_func, logMHmodel.dMH_dZ, (false, false)),
                                         SFH.GaussianDispersion(disp.σ, (false,)),
                                         smodels, logsdata, logAge, MH, x0=x0)
            @test fresult.map.μ[end-2:end] ≈ [logMHmodel.α, logMHmodel.β, disp.σ]
        ...
        @testset "fit_sfh_fisher" begin
            x0 = SFH.construct_x0_mdf(logAge, convert(T,T_max); normalize_value=sum(logx))
            # Run fit on perfect, noise-free data
            result = SFH.fit_sfh_fisher(SFH.update_params(logMHmodel, (logMHmodel.α + 1e-4, logMHmodel.β + 1e-4)),
                                        SFH.update_params(disp, (disp.σ + 0.1,)),
                                        smodels, logsdata2, logAge, MH,
                                        x0=x0)
            @test result.mle.μ ≈ log_true_vals # With no error, we should converge exactly
            ...
            # Run fit on noisy data
            rresult = SFH.fit_sfh_fisher(SFH.update_params(logMHmodel, (logMHmodel.α + 1e-4, logMHmodel.β + 1e-4)),
                                         SFH.update_params(disp, (disp.σ - 0.1,)),
                                         smodels, logsdata, logAge, MH, x0=x0)
            # Test that MLE and MAP results are within 3σ of the true answer for all parameters
            @test all(isapprox(rresult.mle.μ[i], log_true_vals[i];
            ...
            # Run with fixed parameters on noisy data, verify that best-fit values are unchanged
            fresult = SFH.fit_sfh_fisher(SFH.LogarithmicAMR(logMHmodel.α, logMHmodel.β, logMHmodel.T_max,
                                                             logMHmodel.MH_func, logMHmodel.dMH_dZ, (false, false)),
                                         SFH.GaussianDispersion(disp.σ, (false,)),
                                         smodels, logsdata, logAge, MH, x0=x0)
            @test fresult.map.μ[end-2:end] ≈ [logMHmodel.α, logMHmodel.β, disp.σ]
