...
@testset "Logarithmic AMR, Free σ" begin
    ...
    @testset "fit_templates_logamr fixed σ" begin
        ...
        result = SFH.fit_templates_logamr(models, data, logAge, MH, σ; x0=x0_1, MH_func=SFH.MH_from_Z, MH_deriv_Z=SFH.dMH_dZ, max_logAge=log10(max_age*1e9))
        # Test that MLE and MAP results are within 3σ of the true answer for all parameters
        ...
        # Now test under perfect data
        data2 = sum(x .* models) # Perfect data, no noise
        result2 = SFH.fit_templates_logamr(models, data2, logAge, MH, σ; x0=x0_1, MH_func=SFH.MH_from_Z, MH_deriv_Z=SFH.dMH_dZ, max_logAge=log10(max_age*1e9))
        @test result2.mle.μ ≈ true_params_1 rtol=1e-5
        @test result2.map.μ ≈ true_params_1 rtol=1e-2
        
        # Now test with stacked models / data
        result3 = SFH.fit_templates_logamr(SFH.stack_models(models), vec(data2), logAge, MH, σ; x0=x0_1, MH_func=SFH.MH_from_Z, MH_deriv_Z=SFH.dMH_dZ, max_logAge=log10(max_age*1e9))
