...
@testset verbose=true "StarFormationHistories.jl" begin
    @testset verbose=true "CMD Simulation" begin
        ...
        @testset "generate_stars_mass" begin
            rng = StableRNG(seedval)
            for i in eachindex(float_types, float_type_labels)
                label = float_type_labels[i]
                @testset "$label" begin
                    ...
                    result = SFH.generate_stars_mass(m_ini, mags, mag_names, total_mass, imf; dist_mod=T(20), rng=rng, mag_lim=T(Inf), mag_lim_name=mag_names[2], binary_model=SFH.NoBinaries())
                    ...
                    # For `total_mass=1e5`, rtol=0.01 is almost too small...
                    @test reduce(+,reduce(+,result[1])) ≈ total_mass * mass_frac rtol=1e-2
                    
                    # Test with RandomBinaryPairs() model
                    result = SFH.generate_stars_mass(m_ini, mags, mag_names, total_mass, imf; dist_mod=T(20), rng=rng, mag_lim=T(Inf), mag_lim_name=mag_names[2], binary_model=SFH.RandomBinaryPairs(T(4//10)))
                    @test result[1] isa Vector{SVector{2,T}}
                    @test result[2] isa Vector{SVector{2,T}}
                    @test length(result[1]) == length(result[2])
                    # println((reduce(+,reduce(+,result[1])) .- total_mass * mass_frac * 14//10) / total_mass / mass_frac / 14//10)
                    @test reduce(+,reduce(+,result[1])) ≈ total_mass * mass_frac * 14//10 rtol=1e-2

                    # Test with BinaryMassRatio() model
                    result = SFH.generate_stars_mass(m_ini, mags, mag_names, total_mass, imf; dist_mod=T(20), rng=rng, mag_lim=T(Inf), mag_lim_name=mag_names[2], binary_model=SFH.BinaryMassRatio(T(4//10), Uniform(T(1//10), T(1))))
                    @test result[1] isa Vector{SVector{2,T}}
                    @test result[2] isa Vector{SVector{2,T}}
                    @test length(result[1]) == length(result[2])
        ...
        @testset "generate_stars_mag" begin
