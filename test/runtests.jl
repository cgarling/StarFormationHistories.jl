        ...
        @testset "logplr" begin
            for i in eachindex(float_types, float_type_labels)
                ...
                @testset "$label" begin
                    ...
                    @test SFH.logplr( C, data ) ≈ -0.5672093513510137 rtol=rtols[i]
                    @test SFH.logplr( C, data ) isa T
                    # Test for 3-argument signature
                    A = T[1 1 1; 0 0 0; 0 0 0]
                    B = T[0 0 0; 1 1 1; 1.5 1.5 1.5]
                    models = [A,B]
                    coeffs = T[1,2]
                    @test SFH.logplr(coeffs, models, data) ≈ -0.5672093513510137 rtol=rtols[i]
                    @test SFH.logplr(coeffs, models, data) isa T
                    # Test 2-argument call signature for flattened input
                    C2 = T[1,2,3,1,2,3,1,2,3]
                    data2 = Int64[1,2,2,1,2,2,1,2,2]
                    @test SFH.logplr( C2, data2 ) ≈ -0.5672093513510137 rtol=rtols[i]
                    @test SFH.logplr( C2, data2 ) isa T
                    # Test 3-argument call signature for flattened input
                    A2 = T[1,0,0,1,0,0,1,0,0]
                    B2 = T[0,1,1.5,0,1,1.5,0,1,1.5]
                    models2 = [A2 B2]
                    coeffs = T[1,2]
                    @test SFH.logplr(coeffs, models2, data2) ≈ -0.5672093513510137 rtol=rtols[i]
                    @test SFH.logplr(coeffs, models2, data2) isa T
                end
            end
        end
        @testset "∇logplr" begin
            for i in eachindex(float_types, float_type_labels)
                ...
                @testset "$label" begin
                    ...
                    C = T[1 1 1; 2 2 2; 3 3 3]
                    data = Int64[1 1 1; 2 2 2; 2 2 2]
                    result = SFH.∇logplr( model, C, data )
                    ...
                    models = [ T[1 1 1; 0 0 0; 0 0 0],
                               ...
                               T[0 0 0; 0 0 0; 1 1 1] ]
                    coeffs = T[1.5, 3, 3]
                    result = SFH.∇logplr( coeffs, models, data )
                    ...
                    coeffs = T[1.5, 3, 3]
                    result = SFH.∇logplr( coeffs, models2, data2 )
                    @test result ≈ [-1, -1, -1] rtol=rtols[i]
                    @test result isa Vector{T}
                    @test length(result) == 3
                end
        ...
        @testset "∇logplr!" begin
            for i in eachindex(float_types, float_type_labels)
                ...
                @testset "$label" begin
                    ...
                    SFH.∇logplr!( grad2, C2, models2, data2 )
