            ...
            @testset "Basic Linear Combinations" begin
                ...
                let x0=T[1], models=[T[0 0 0; 0 0 0; 1 1 1]], data=Int64[0 0 0; 0 0 0; 3 3 3], C=zeros(T,3,3)
                    ...
                    lbfgsb_result2 = SFH.fit_templates_lbfgsb(smodels,
                                                              sdata; composite=sC, x0=x0, iprint=-1)
                    @test lbfgsb_result2[2][1] ≈ 3 rtol=tset_rtol
                    # Test fit_templates
                    ft_result = SFH.fit_templates(models, data; composite=C, x0=x0)
                    @test ft_result.mle.μ[1] ≈ 3 rtol=tset_rtol
                    # Test fit_templates flattened call signature
                    ft_result2 = SFH.fit_templates(smodels, sdata; composite=sC, x0=x0)
                    @test ft_result2.mle.μ[1] ≈ 3 rtol=tset_rtol
                    # Test fit_templates_fast
                    ftf_result = SFH.fit_templates_fast(models, data; composite=C, x0=x0)
                    @test ftf_result[1][1] ≈ 3 rtol=tset_rtol
                    # Test fit_templates_fast flattened call signature
                    ftf_result2 = SFH.fit_templates_fast(smodels, sdata; composite=sC, x0=x0)
                ...
                let x=rand(rng,T,N_models), x0=rand(rng,T,N_models), models=[rand(rng,T,hist_size...) for i in 1:N_models], data=sum(x .* models), C=zeros(T,hist_size)
                    ...
                    sdata2 = vec(data2)
                    lbfgsb_result2 = SFH.fit_templates_lbfgsb(smodels,
                                                              sdata2; composite=sC, x0=x0, iprint=-1)
                    @test lbfgsb_result2[2] ≈ x2 rtol=tset_rtol
                    # Test fit_templates
                    ft_result = SFH.fit_templates(models, data2; composite=C, x0=x0)
                    @test ft_result.mle.μ ≈ x2 rtol=tset_rtol
                    # Test fit_templates flattened call signature
                    ft_result2 = SFH.fit_templates(smodels, sdata2; composite=sC, x0=x0)
                    @test ft_result2.mle.μ ≈ x2 rtol=tset_rtol                    
                    # Test fit_templates_fast
                    ftf_result = SFH.fit_templates_fast(models, data2; composite=C, x0=x0)
                    @test ftf_result[1] ≈ x2 rtol=tset_rtol
                    # Test fit_templates_fast flattened call signature
                    ftf_result2 = SFH.fit_templates_fast(smodels, sdata2; composite=sC, x0=x0)
