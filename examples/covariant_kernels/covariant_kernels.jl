...
sep_bins = (range(start=x0_sep - 3*x0_err,stop=x0_sep + 3*x0_err_sep,length=100),
            range(start=y0_sep - 3*y0_err,stop=y0_sep + 3*y0_err_sep,length=100))
...
covar1_matrix = SMatrix{2,2}( ((x0_err_covar1 / step(covar1_bins[1]))^2 + (y0_err_covar1 / step(covar1_bins[2]))^2),
                              ...
                              (y0_err_covar1 / step(covar1_bins[2]))^2,
                              # 0.0, 0.0,
                              # 0.00148198 / step(covar1_bins[2])^2, 0.00148198 / step(covar1_bins[2])^2,
                              # -4.35461e-7 / step(covar1_bins[2])^2, -4.35461e-7 / step(covar1_bins[2])^2,
                              (y0_err_covar1 / step(covar1_bins[2]))^2 )
covar1_matrix = cov([x_mags_covar1 y_mags_covar1]) ./ step(covar1_bins[1])^2
covar1_kernel = SFH.Gaussian2D( length(covar1_bins[1])/2, length(covar1_bins[2])/2, covar1_matrix)
...
k1 = heatmap(sep_bins[1], sep_bins[2], sep_kernel_img)
diff1 = heatmap(sep_bins[1], sep_bins[2], sep_data_hist.weights .- sep_kernel_img)
# First covariant kernel
r2 = heatmap(covar1_data_hist.edges[1],covar1_data_hist.edges[2],covar1_data_hist.weights')
k2 = heatmap(covar1_bins[1], covar1_bins[2], covar1_kernel_img')
diff2 = heatmap(covar1_bins[1], covar1_bins[2], covar1_data_hist.weights' .- covar1_kernel_img')
