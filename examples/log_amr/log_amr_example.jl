import StarFormationHistories: Z_from_MH, MH_from_Z, calculate_coeffs_logamr
using Plots
gr()
...
β::Float64 = Z_from_MH(-0.8) # Present-day metal mass ratio
α::Float64 = (Z_from_MH(-2.5) - β) / 13.7 # Slope of AMR; Z = t * α + β // Units of [1/Gyr]
...
relative_weights = calculate_coeffs_logamr(ones(length(unique_logAge)), logAge, MH, α, β, σ)
...
plot!(exp10.(unique_logAge) ./ 1e9, β .+ α .* (exp10.(unique_logAge) ./ 1e9); c="black", ls=:dash)
...
plot!(unique_logAge, MH_from_Z.(β .+ α .* (exp10.(unique_logAge) ./ 1e9)); c="black", ls=:dash)
...
plot!(exp10.(unique_logAge) ./ 1e9, MH_from_Z.(β .+ α .* (exp10.(unique_logAge) ./ 1e9)); c="black", ls=:dash)
