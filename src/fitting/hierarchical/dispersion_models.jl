...
"""
    npar(model::AbstractDispersionModel)::Int
Returns the number of fittable parameters in the model. 
"""
npar(model::AbstractDispersionModel)
"""
    gradient(model::AbstractDispersionModel{T}, x::Real, μ::Real)::NTuple{npar(model)+1, T}
...
"""
    transforms(model::AbstractDispersionModel)::NTuple{npar(model), Int}
...
"""
    free_params(model::AbstractDispersionModel)::NTuple{npar(model), Bool}
...
julia> npar(GaussianDispersion(0.2)) == 1
true
...
npar(model::GaussianDispersion) = 1
