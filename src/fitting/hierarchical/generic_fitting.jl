...
# Implementation
This method splits the requested number of samples `Nsamples` into a number of independent HMC chains, each of which has length `chain_length`. Initial positions for each chain are randomly drawn from the multivariate Gaussian approximation to the objective function stored in `bfgs_result`, approximating a warm start. Smaller values of `chain_length` achieve better load balancing while larger values of `chain_length` allow each chain more time to mix (see also [Chen2019](@cite)). The default value of 100 results in good mixing with 24 fitting variables and a well-scaled step length ϵ -- higher dimensional problems should increase `chain_length`. The downside to large `chain_length` is poor load balancing across available threads resulting in longer runtimes.
