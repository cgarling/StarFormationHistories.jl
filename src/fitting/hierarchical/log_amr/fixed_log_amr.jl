...
"""
    fixed_log_amr(models, data, logAge, metallicities, constraint1, constraint2, σ; Z_func=Z_from_MH, max_age=maximum(logAge), kws...)
Call signature that takes two fixed points `low_constraint` and `high_constraint` that define points that must lie on the logarithmic age-metallicity relation and calculates the paramters α and β for you. Format is ([M/H], age [Gyr]), i.e. `constraint1 = (-2.5,13.7)` for the first point at [M/H] = -2.5 at 13.7 Gyr lookback time and `constraint2 = (-0.8,0.0)` for the second point at [M/H] = -0.8 at present-day (0.0 Gyr lookback time). By default it is assumed that one of these points is the earliest time you are interested in, at which by definition `Z=β`. You can provide a larger lookback time to normalize β by providing a larger `max_age` (units of Gyr). Metallicities in [M/H] iare converted to metal mass fractions Z via the provided callable keyword argument `Z_func` which defaults to [`StarFormationHistories.Z_from_MH`](@ref).
"""
function fixed_log_amr(models, data, logAge, metallicities, low_constraint, high_constraint, σ; Z_func=Z_from_MH, max_age=max(last(low_constraint),last(high_constraint)), kws...)
