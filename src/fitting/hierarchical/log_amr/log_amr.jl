...
Calculates per-model stellar mass coefficients `coeffs` from the fitting parameters of [`StarFormationHistories.fit_templates_logamr`](@ref) and [`StarFormationHistories.hmc_sample_logamr`](@ref). The `variables` returned by these functions is of length `length(unique(logAge))+3`. The first `length(logAge)` entries are stellar mass coefficients, one per unique entry in `logAge`. The final three elements are α, β, and σ defining a metallicity evolution such that the mean metal mass fraction Z for element `i` of `unique(logAge)` is `μ_Z[i] = α * (T_max - exp10(unique(logAge)[i]) / 1e9) + β`. This is converted to a mean metallicity in [M/H] via the provided callable keyword argument `MH_func` which defaults to [`StarFormationHistories.MH_from_Z`](@ref). The individual weights per each isochrone are then determined via Gaussian weighting with the above mean [M/H] and the provided `σ` in dex. The provided `metallicities` vector should be in [M/H]. 
...
"""
function calculate_coeffs_logamr(variables::AbstractVector{<:Number}, logAge::AbstractVector{<:Number}, metallicities::AbstractVector{<:Number}, T_max::Number, α::Number, β::Number, σ::Number; MH_func=MH_from_Z)
    @assert (α >= 0) & (β >= 0) & (σ > 0)
    max_logAge = log10(T_max) + 9 # T_max in units of Gyr
    if maximum(logAge) > max_logAge
        @warn "We recommend that the argument `T_max` to `StarFormationHistories.calculate_coeffs_logamr` be set equal to or greater than `exp10(maximum(logAge)-9)`. The provided `T_max` is less than this, such that it is possible the metal mass fraction may become negative in the model, which would be unphysical."
    end
    S = promote_type(eltype(variables), eltype(logAge), eltype(metallicities), typeof(T_max), typeof(α), typeof(β), typeof(σ))
    ...
    for i in eachindex(unique_logAge)
        la = unique_logAge[i]
        μZ = α * (T_max - exp10(la - 9)) + β # Find the mean Z of this age bin
...
@inline function fg_logamr!(F, G, variables::AbstractVector{<:Number}, models::Union{AbstractMatrix{<:Number}, AbstractVector{<:AbstractMatrix{<:Number}}}, data::Union{AbstractVector{<:Number}, AbstractMatrix{<:Number}}, composite::Union{AbstractVector{<:Number}, AbstractMatrix{<:Number}}, logAge::AbstractVector{<:Number}, metallicities::AbstractVector{<:Number}, T_max::Number, MH_func, MH_deriv_Z)
    ...
    coeffs = calculate_coeffs_logamr(view(variables,firstindex(variables):lastindex(variables)-3), logAge, metallicities, T_max, α, β, σ; MH_func=MH_func)
    ...
    if (F != nothing) & (G != nothing) # Optim.optimize wants objective and gradient
        ...
        for i in axes(G,1)[begin:end-3] 
            ...
            meanZ = α * (T_max - age) + β # Mean metal mass fraction
            ...
            dZdα = (T_max - age) # Derivative of metal mass fraction with respect to α
            dLdβ = dAdμ * dμdZ * dZdβ
            dLdα = dLdβ * dZdα
...
function fit_templates_logamr(models::AbstractMatrix{S},
    ...
    # All variables must be positive  since Z = α * (T_max - age) + β with age being
    # positive lookback time in Gyr, α must always be positive for an increasing AMR.
    ...
    function fg_logamr!_map(F, G, xvec)
        ...
        logL = fg_logamr!(F, G, x, models, data, composite,
                          logAge, metallicities, T_max, MH_func, MH_deriv_Z)
...
fit_templates_logamr(models::AbstractVector{<:AbstractMatrix{<:Number}}, data::AbstractMatrix{<:Number}, logAge::AbstractVector{<:Number}, metallicities::AbstractVector{<:Number}, T_max::Number; kws...) = fit_templates_logamr(stack_models(models), vec(data), logAge, metallicities, T_max; kws...)


# Calculate loglikelihood and gradient with respect to SFR parameters and logarithmic AMR parameters
@inline function fg_logamr_fixedσ!(F, G, variables::AbstractVector{<:Number}, models::Union{AbstractMatrix{<:Number}, AbstractVector{<:AbstractMatrix{<:Number}}}, data::Union{AbstractVector{<:Number}, AbstractMatrix{<:Number}}, composite::Union{AbstractVector{<:Number}, AbstractMatrix{<:Number}}, logAge::AbstractVector{<:Number}, metallicities::AbstractVector{<:Number}, T_max::Number, MH_func, MH_deriv_Z, σ::Number)
    ...
    unique_logAge = unique(logAge)
    # Calculate the per-template coefficents and normalization values
    coeffs = calculate_coeffs_logamr(view(variables,firstindex(variables):lastindex(variables)-2), logAge, metallicities, T_max, α, β, σ; MH_func=MH_func)
...
function fit_templates_logamr(models::AbstractMatrix{S},
    ...
    function fg_logamr!_map(F, G, xvec)
        ...
        logL = fg_logamr_fixedσ!(F, G, x, models, data, composite,
                          logAge, metallicities, T_max, MH_func, MH_deriv_Z, σ)
        # Appy Jacobian correction to logL; α should be the same as
        # D[Log[x],x] = D[Log[-x],x] = 1/x, Log[1/x] = -Log[x]
        logL -= sum( xvec ) 
        # Add the Jacobian correction for every element of G
...
fit_templates_logamr(models::AbstractVector{<:AbstractMatrix{<:Number}}, data::AbstractMatrix{<:Number}, logAge::AbstractVector{<:Number}, metallicities::AbstractVector{<:Number}, T_max::Number, σ::Number; kws...) = fit_templates_logamr(stack_models(models), vec(data), logAge, metallicities, T_max, σ; kws...)
