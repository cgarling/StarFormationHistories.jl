...
function fisher_information!(I_mat::AbstractMatrix{T}, 
                             W::AbstractMatrix{T}, 
                             models::AbstractMatrix{<:Number}, 
                             composite::AbstractVector{<:Number}) where {T}

    @argcheck axes(composite, 1) == axes(models, 1)
    @argcheck axes(W) == axes(models)
    @argcheck size(I_mat) == (size(models, 2), size(models, 2))

    @turbo thread=false for j in axes(models, 2)
        for i in axes(models, 1)
            @inbounds mi = max(composite[i], eps(T))
            @inbounds W[i, j] = models[i, j] / sqrt(mi)
        end
    end
    mul!(I_mat, transpose(W), W) # This is basically all the cost
    return Symmetric(I_mat)
end

"""
    fisher_information(models::AbstractMatrix{<:Number}, composite::AbstractVector{<:Number})

Computes the Fisher information matrix for the Dolphin (2002) Poisson likelihood ratio
with respect to the per-template coefficients ``r_j``.

The Fisher information is
```math
I_{jk} = -\\mathrm{E}\\!\\left[\\frac{\\partial^2 \\ln \\mathscr{L}}{\\partial r_j \\, \\partial r_k}\\right]
        = \\sum_i \\frac{c_{i,j} \\, c_{i,k}}{m_i}
```
where ``m_i = \\sum_j r_j c_{i,j}`` is the composite model (see [`composite!`](@ref)) and
``c_{i,j}`` is element ``i`` of template ``j``.

Programmatically, this equals ``\\mathbf{C}^\\top \\mathbf{D} \\mathbf{C}`` where
``\\mathbf{D} = \\mathrm{Diag}(1/\\mathbf{m})``. We factor this as
``\\mathbf{W}^\\top \\mathbf{W}`` where ``W_{ij} = c_{ij} / \\sqrt{m_i}``, which
exploits the BLAS symmetric rank-``k`` update (via `mul!`) and avoids constructing the
full diagonal weight matrix. The result is returned as a `Symmetric` matrix.
"""
function fisher_information(models::AbstractMatrix{<:Number}, composite::AbstractVector{<:Number})
    @argcheck axes(composite, 1) == axes(models, 1)
    T = promote_type(eltype(models), eltype(composite))
    W = similar(models, T)
    nmod = size(models, 2)
    I_mat = Matrix{T}(undef, nmod, nmod)
    return fisher_information!(I_mat, W, models, composite)
end
